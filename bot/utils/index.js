...
<i>🛒 Barcha Mahsulotlar 👇👇👇</i>`;
        ...
        products.forEach((item, i) => {
            text += `\n-----------------\n${item?.product?.shop?.name} 🛍️${item?.product?.name} - ${parseInt(item.quantity)} x ${parseInt(item?.product?.sale_price)} = ${parseInt(item?.product?.sale_price) * parseInt(item.quantity)} so'm\n`;
            buttons.push(
